...
输出端通过电阻 Rf 连回反相输入端（−），反相端再通过 R1 接地；信号接同相端（+）。

**推导增益**

由虚断，流入 V₋ 的电流为 0，所以流过 R1 和 Rf 的电流相等，设为 I：

$$V_- = I \cdot R_1$$
$$V_{out} = I \cdot (R_1 + R_f)$$
...
电压跟随器是同相放大器 $R_f = 0$，$R_1 = \infty$ 的特殊情况。
...
信号通过 Rin 接到反相输入端（−），同相端（+）接地，Rf 跨接在输出和反相端之间。
...
- **I/V 转换器**：$R_{in}$ 用电流源替代，输出电压正比于输入电流，光电二极管、电流型传感器的标准前端
