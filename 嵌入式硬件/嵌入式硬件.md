...
#### TVS 二极管
有单向和双向。单向 TVS 符号与 ZD稳压二极管 完全相同。它们和普通二极管的区别就在于反向击穿后还可以工作在一定的功率内，电压变化小，所以约等于稳压作用。

- [稳压二极管](https://b23.tv/uYIScoV)

| | ZD 稳压二极管 | TVS 二极管 |
|--|--|--|
| 符号 | 相同（单向） | 相同（单向） / 独有（双向） |
| 功率 | 小（mW 级） | 大（W～kW 级） |
| 用途 | 稳压基准 | 瞬态过压保护 |
| 响应速度 | 慢 | ps 级 |
...
|---|---|---|
|控制方式|**电压**控制（栅极）|**电流**控制（基极）|
|输入阻抗|极高（GΩ级）|较低（kΩ级）|
|驱动难度|容易，几乎不耗电|需要持续提供基极电流|
|开关速度|极快|较快，但略逊|
|线性放大|较弱|**非常好**|
|噪声特性|较大|较小|
|集成度|**极高**，适合大规模集成|占面积较大|
|成本|大量生产后很低|分立器件成本低|
MOS 管"占主导"的领域：
- **数字电路 / CPU / 内存**：几乎 100% 是 MOSFET（CMOS 工艺），因为静态功耗极低、集成度极高
- **电源开关（大功率）**：功率 MOSFET 驱动方便、效率高，广泛用于开关电源、电机驱动
- **手机、平板等低功耗设备**：CMOS 逻辑门是基础

三极管（BJT）不可替代的领域：
- **模拟放大电路**：音频放大、仪器仪表，BJT 的线性度和低噪声特性更优
- **精密电流源 / 镜像电路**：BJT 的 Vbe 特性非常稳定，适合做基准
- **射频（RF）电路**：某些高频低噪声放大器（LNA）仍优先选 BJT
- **驱动级电路**：在需要精确电流控制的场景下更直观

---

## 一句话总结

> **数字电路和大功率开关 → MOS 管几乎全面胜出； 模拟放大、低噪声、精密线性电路 → BJT 仍有不可替代的地位。**

两者并非替代关系，而是在各自擅长的赛道上共存。现代很多芯片（如 BiCMOS 工艺）甚至把两者集成在一起，取长补短。
