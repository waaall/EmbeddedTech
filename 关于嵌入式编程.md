...
>
> 因此这款芯片既非典型的单片机也非典型的嵌入式处理器，但兼具两者特点，所以NXP将其称为"跨界处理器"。
>
> 关于NXP的设计思路：单片机内置Flash成本较高，imxRT1052相比同性能的STM32便宜很多，主要原因在于它没有内置Flash。
>
> 因此有人认为ST落后于NXP，但这种说法不够准确。NXP的这种设计在单片机领域是一种创新，对比之下STM32F7、STM32H7的性价比显得不足。客观而言，STM32的F7和H7系列定价确实偏高，单片售价过百元的定价策略令人费解。
>
> 回到跨界处理器imxRT1052，该芯片主频达600MHz，有消费级和工业级版本，外设丰富（基本覆盖常见应用需求），支持最大分辨率1024×768的TFT LCD（需外接SDRAM作为显存）。相较于普通单片机，这款处理器性能极为强劲。
>
> 价格方面，imxRT1052官方批量价格约30元人民币，性价比较高。STM32也并非处于劣势，F7和H7的主频虽不及600MHz，但H7的整体性能与RT1052相差不大。
>
> F7和H7未能大规模普及的关键原因在于定价过高，百元以上的单价让许多开发者望而却步。相比之下，imxRT1052约30元的价格确实具有吸引力，因此有人据此判断NXP将在市场上超越ST。
>
> **跨界处理器的尴尬**
>
...
- **代系（Architecture Version）**：ARMv4、v5、v6、**v7**、**v8**、**v9**…这是**指令集架构（ISA）版本**。新一代通常引入指令集和系统级能力的大升级（如 v7 的 Thumb-2，v8-A 的 64 位 AArch64，v8-M 的 TrustZone-M，v9-A 的 SVE2/MTE/RME 等）。
    
- **分型（Profile）**：自 v7 正式分为 **A-profile（应用处理器）** / **R-profile（实时）** / **M-profile（微控制器）**，各自面向不同场景，硬件能力不同：A 有 MMU 跑完整 OS；R 强实时/确定性；M 聚焦低功耗与易用 MCU。
    
- **内核型号（Core）**：如 Cortex-A8、Cortex-M4、Cortex-M33，是**具体微架构实现**，各自“落在”某一代系和某一分型上。
    
- **SoC/MCU 型号**：各芯片厂（STM、NXP、Renesas…）在某个内核上做的芯片产品（如 STM32F4=基于 M4；LPC55Sxx=基于 M33），外设/存储/工艺由厂商决定。
...
| **代系（ISA）**   | **A-profile（应用）**                                                          | **R-profile（实时）** | **M-profile（微控）**               | **该代系的关键特征**                                                  |
| ------------- | -------------------------------------------------------------------------- | ----------------- | ------------------------------- | ------------------------------------------------------------- |
| **ARMv6**     | ARM11                                                                      | –                 | –                               | 引入 Thumb-2 的前夜；多数仍 32 位 AArch32                               |
| **ARMv7**     | **A5、A7、A8、A9、A12、A15、A17**                                                | **R4、R5、R7、R8**   | **M0/M0+/M1、M3、M4、M7**          | Thumb-2 普及；A/R 一般带 MMU，M 走 Thumb-only/MPU 可选，M4 带 DSP/FPU 可选。 |
| **ARMv8-A**   | **A32（仅 32 位）、A35、A53、A57、A72、A73、A75、A76、A77、A78** 等（后续过渡到 v8.x-A / v9-A） | –                 | –                               | **AArch64**（64 位）加入，同时保留 AArch32；NEON 强化、虚拟化/安全扩展演进。          |
| **ARMv8-R**   | –                                                                          | **R52/R52+、R82**  | –                               | 在实时领域引入更强的隔离/虚拟化等（EL0/EL1 概念），安全与确定性并重。                       |
| **ARMv8-M**   | –                                                                          | –                 | **M23（Baseline）、M33（Mainline）** | **TrustZone-M**、Baseline/Mainline 分支；与 v6-M 兼容性良好。            |
| **ARMv8.1-M** | –                                                                          | –                 | **M55、M85（带 Helium/MVE）**       | **Helium=M-profile 向量扩展（MVE）**，显著提升 DSP/ML。                   |
| **ARMv9-A**   | **X2/A710/A510 起步，后续 A715/A720…**                                          | –                 | –                               | **SVE2 矢量、MTE 内存标签、RME 机密计算** 等安全/AI 能力。                      |
...
#### 2.2.1 [乐鑫解决方案](https://www.espressif.com.cn/zh-hans/home)

#### 2.2.2 开发板的选择

与芯片选型类似，开发板不必追求高性能，但旧型号（性能较差）不一定便宜，因为部分芯片已停产。类比内存市场，DDR3价格并不比DDR4低（2020年后）。此外，过于陈旧的开发板可能存在系统和协议支持不足的问题。**STM32系列紧随ARM Cortex-M架构更新，是非ARM-Linux方案的首选**。
...
虽是基础内容，但[这个视频](https://www.bilibili.com/video/BV1w4411B7a4)讲解较为清晰。可使用`gcc -o ***.c -v`查看编译过程，视频6分钟处介绍了[gcc编译时生成依赖文件](https://www.bilibili.com/video/BV1w4411B7a4)的方法。
...
许多教程仍在使用旧的系统工具，如net-tools（已于2001年停止维护）。当前系统默认使用iproute2和netplan进行网络查询和配置。

具体讲解见:[Ubuntu20.04网络配置](https://www.360blogs.top/ubuntu20-04-netplan/)

配置netplan时，服务器端使用networkd即可，无需使用NetworkManager（两者区别可查阅相关文档）。

>  **[硬软件工具](https://www.bilibili.com/video/BV1F7411u71s)**
>
>  工业级芯片电路图绘制工具：立创EDA（国产开源）和KiCad（开源）。
>
>  简单电路图可使用Python + Schemdraw绘制。



#### 3.5.2 Schemdraw（Python电路图绘制库）
...
[乐鑫Wi-Fi/蓝牙解决方案](https://www.espressif.com.cn/zh-hans/home)
...
工业级芯片电路图绘制工具推荐：立创EDA（国产开源）和KiCad（开源）。
...
>  | [bleak](https://github.com/hbldh/bleak.git)              | win/linux  | >=4.0                                        | 功能完善，支持使用通用蓝牙适配器建立BLE通信 |
>  | WCH                                                      | win/linux  | >=4.0                                        | 动态链接库，功能较全，文档待完善 |
...
Typora和Notion收费后，可使用VS Code编辑Markdown。安装"Markdown All in One"插件即可获得良好的编辑体验。

> [Markdown All in One文档](https://github.com/yzhang-gh/vscode-markdown#keyboard-shortcuts)
>
> 如需中文界面，还需安装Chinese语言包插件。
...
#### Linux安装时报错：cannot acquire the dpkg frontend lock
